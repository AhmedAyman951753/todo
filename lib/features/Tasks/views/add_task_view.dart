...
class AddTask extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(create: (context) => AddTaskCubit(),
      child: Scaffold(
            ...
            child: Column(
              children: [
                ...
                Row(
                  children: [
                    IconButton(
                      ...
                      onPressed: (){myNavigator(context, screen: Home());}
                      ),
                    SizedBox(width: 105),
                ...
                Builder(builder: (context) => Column(
                  children: [
                    TextContainer(label: "Task Name", hint: "Enter task name", borderColor: AppColors.green, textController: AddTaskCubit.get(context).titleController),
                    SizedBox(height: 20),
                    TextContainer(label: "Description", hint: "Enter task description ...", borderColor: AppColors.green, textController: AddTaskCubit.get(context).descriptionController),
                    SizedBox(height: 20),
                  ],
                )),
                BlocConsumer<AddTaskCubit, AddTaskState>(builder: (context, state)
                    {
